...
if len(sys.argv) > 2:
    ...
    with open(sys.argv[2]) as f:
        ...
        matrix = list(map(lambda l: list(filter(lambda x: x != '', l)),
                          contents_split))
        ...
        for i in range(n):
            for j in range(n):
                e = matrix[i][j]
                match = re.search(r'\d+',
                                  e)  # verifie que l'entree est une sequence de chiffres
                if match:
                    v = int(e)
                    # print("ajout de %1d pour (%1d,%1d)" % (v,i,j)) 
                    cnf.append([vpool.id((i, j,
                                          v))])  # ajout d'une liste ne contenant qu'un seul literal
...
def affichage_solution(interpretation):
    for i in range(n):
        ...
        for j in range(n):
            for v in range(n):
                if vpool.id((i, j,
                             v + 1)) in interpretation_filtree:  # la repetition de cette etape prend du temps si on ne filtre pas avant
                    if (j % c) == 0 and j != 0:
                        print(" | %2d " % (v + 1), end='')
                    else:
                        print("%2d " % (v + 1), end='')
    ...
    if test_unicite:
        ...
        for i in range(n):
            for j in range(n):
                for v in range(n):
                    if vpool.id(
                            (i, j, v + 1)) in interpretation_filtree:
                        d.append(-vpool.id((i, j, v + 1)))
        s.add_clause(d)
