...
def solution(m1, m2):
            ...
            for j in range(column_quantity):
                for d in CARDINALS:
                    if 0 <= i + 2 * d[
                        ...
                        1] < column_quantity:
                        clauses.append(vpool.id((i, j, d, s-1))) # s-1, car nous utilisons la boucle for s in range(1, steps_quantity + 1), mais allons de s=0 à s-1
        cnf.append(clauses)
    ...
    solver = Glucose4(use_timer=True)  # pour utiliser le solveur MiniSAT
